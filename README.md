...
progress in `localStorage`, all speech scoring from the browser's recognizer.
The interviewer's questions play from pre-generated Hungarian audio; everything
personalized falls back to the browser's speech synthesis. It installs to a
...
Speaking drills use the Web Speech API in the other direction:
